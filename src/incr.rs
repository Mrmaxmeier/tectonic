// src/incr.rs -- Command-line driver for the Tectonic engine.
...
fn inner(matches: ArgMatches, config: PersistentConfig, status: &mut TermcolorStatusBackend) -> Result<i32> {
    ...
    watcher.watch(matches.value_of("INPUT").unwrap(), RecursiveMode::NonRecursive).unwrap();
...
fn main() {
    ...
    if env::var("SOURCE_DATE_EPOCH").is_err() {
        env::set_var("SOURCE_DATE_EPOCH", "1456304492");
        tt_note!(status, "settting SOURCE_DATE_EPOCH=1456304492 for semi-reproducible pdf output");
        // status.note_highlighted("note: settting ", "SOURCE_DATE_EPOCH=1456304492", " for semi-reproducible pdf output");
    }
